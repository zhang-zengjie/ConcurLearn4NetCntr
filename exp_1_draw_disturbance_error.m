...
plot(time(1:2:end), beta(:,1:2:end) - hat_beta_conv(:,1:2:end),'LineWidth',1.5,'LineStyle','-.');
axis([0,time(end),-0.60001,0.60001]);
line([0,time(end)],[0,0],'LineWidth',1.2,'LineStyle','-.','color','k');
ax = gca;
ax.XTick = 0:1:5;
ylabel('$\tilde{d}_i(k)$','Interpreter','latex', 'FontSize', 11);
grid on;
...
hold on;
set(0, 'DefaultFigureRenderer', 'painters')
plot(time(1:2:end), beta(:,1:2:end) - hat_beta(:,1:2:end),'LineWidth',1.5,'LineStyle','-.');
axis([0,time(end),-0.60001,0.60001]);
line([0,time(end)],[0,0],'LineWidth',1.2,'LineStyle','-.','color','k');
ax = gca;
ax.XTick = 0:1:5;
...
ax.YTick = 0:50:100;
ax.XTick = 0:1:5;
ylabel('$n_s$','Interpreter','latex', 'FontSize', 11);
grid on;
set(gca,'GridLineStyle','-.', 'FontSize', 11);
xlabel('time (s)','Interpreter','latex', 'FontSize', 11);
x0=500;
