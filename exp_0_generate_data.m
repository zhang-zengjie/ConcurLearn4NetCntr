...
d = zeros(N,T);              % Generate disturbance
for i=1:T
    d(:,i) = d_bar*(1+0.5*sin(2*time(i)));
end
...
for k= 1:T
    ...
    x(:,k+1) = x(:,k) + h*(L*d(:,k) - diag(x(:,k))*delta_bar);
    
end

save("exp_0_data.mat", "x", "time", "d");